...
class CplexBestof2Scheduler(Scheduler):
  ...
  def __init__(self, options):
    super(CplexBestof2Scheduler, self).__init__(options)
    ...
    presorter_id = options["scheduler"].get("alternative_presorter", None)
    if presorter_id is None:
      self.purebf_presorter = sorters.sorter_none
    elif presorter_id in sorters.sorters:
      self.purebf_presorter = sorters.sorters[presorter_id]
    else:
    ...
    if self.use_checkpointing:
      ...
      try:
        with open(self.checkpointing_file, 'rb') as read_obj:
      ...
      if os.path.exists(self.checkpointing_file):
  ...
  def _schedule_jobs(self, time, return_plan=False):
    ...
    # Next, get an alternative plan.
    alt_plan = self._purebf_scheduling_plan(queue, time)
  ...
  def _purebf_scheduling_plan(self, queue, time):
    sorted_queue = self.purebf_presorter(queue, time)
