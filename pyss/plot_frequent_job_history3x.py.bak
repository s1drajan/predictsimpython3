...
Usage:
    plot_frequent_job_history3x.py <swf_file>
...
In contrast to plot_frequent_job_history.py,
this file can be used to plot predictions from 3 different predictors.
...
def run_predictor(df):
    predictor1 = Predictor(options=options1)
    predictor2 = Predictor(options=options2)
    predictor3 = Predictor(options=options3)
    list2 = df.sort_values(by=['End Time'])
    list2['prediction_1'] = -1
    list2['prediction_2'] = -1
    list2['prediction_3'] = -1
    for idx in list2.index:
        ...
        avg, var = predictor1.fit(job, job.start_to_run_at_time + job.actual_run_time)
        list2.loc[idx, 'prediction_1'] = avg

        avg, var = predictor2.fit(job, job.start_to_run_at_time + job.actual_run_time)
        list2.loc[idx, 'prediction_2'] = avg

        avg, var = predictor3.fit(job, job.start_to_run_at_time + job.actual_run_time)
        list2.loc[idx, 'prediction_3'] = avg
    return list2
...
def do_main(in_file):
    ...
    # counts = counts[counts['count']==220]
    for _, instance in counts.iterrows():
        ...
        ax1.step(dfp['End Time'], dfp['prediction_1'], where='post', linewidth=2, color='#777777')
        # # show timelimit (optionally)
        # ax.plot([dfp['Start Time'].min(), dfp['End Time'].max()], [limit] * 2, linewidth=.25, color='#777777')

        ax1.step(dfp['End Time'], np.minimum(dfp['prediction_2'], limit), where='post', linewidth=1, color='#999999')
        ax1.step(dfp['End Time'], np.minimum(dfp['prediction_3'], limit), where='post', linewidth=0.5, color='#cccccc')
